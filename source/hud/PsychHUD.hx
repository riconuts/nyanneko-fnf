...
class PsychHUD extends BaseHUD 
{
	...
	override public function new(iP1:String, iP2:String, songName:String, stats:Stats)
	{
		...
		scoreTxt = new FlxText(0, healthBar.y + 48, FlxG.width, "", 20);
		...
		if (ClientPrefs.judgeCounter != 'Off')
		{
			...
			for (idx in 0...displayedJudges.length)
			{
				...
				text.setFormat(Paths.font("vcr.ttf"), 24, judgeColours.get(judgment), LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				text.bold = true;
				...
				numb.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
